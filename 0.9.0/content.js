...
document.addEventListener("click", function (event) {
  const curURL = window.location.hostname; //"about:blank";
  ...
  if (curURL == "vsegdadacom.webim.ru") {
    console.log("Mouse pressed. Key: ", event.target.tagName, ", URL: ", curURL);
    getChatData();
    ...
    if (appDivClasses != null) {
      ...
      if (appDiv != null) {
        chrome.runtime.sendMessage({
          applicationId: appDiv.id
        });
      }
      else {
        console.log("Mouse pressed + Alt");
      }
    }
  }
});
...
document.addEventListener("keydown", function (event) {
  let curURL = window.location.hostname; //"about:blank";
  //console.log("Key pressed 1");
  if (curURL == "sd.vsegda-da.com" && event.code == "KeyV" && event.altKey) {
    console.log("Key pressed. Key: Alt+V ", sdLib);
    chrome.storage.local.get(['sdLib'], result => {
      console.log("Get from store ", result);
      sdLib = result.sdLib;
      console.log("Put to sdLib ", sdLib);
      fillSDForm();
    });
  }
});
...
//<input class="text" type="text" name="summary" id="summary">
function fillSDForm() {
  // Тема
